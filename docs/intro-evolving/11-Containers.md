...
This is the bindings module for all current containers offered by the LUMI AI Factory.
It provides access to all your filespaces and also adds a directory needed for Slurm support.
It also adds an environment variable that is needed to let the MPI library in some of those
containers work properly with Slurm (an environment variable that influences the behaviour
of `srun`).
...
<figure markdown style="border: 1px solid #000">
  ![Environment enhancements (5)](https://465000095.lumidata.eu/training-materials-web/intro-evolving/img/LUMI-BE-Intro-evolving-11-Containers/ContainersEnvironmentEnhancement_5.png){ loading=lazy }
</figure>
