...
!!! Note "AI course"
    The `cotainr` tool is also used extensively in the AI workshop to build containers
    with AI software on top of some 
    [ROCm<sup>TM</sup> containers that we provide](https://lumi-supercomputer.github.io/LUMI-EasyBuild-docs/r/rocm/). A link to the course material of that training was
...
of writing so we cannot link to them).
...
-   We also use a 
    ...
    We use the
    ["Linear assignment of GCD, then match the cores" strategy](http://localhost:8000/LUMI-BE-training-materials/intro-evolving/07-Binding/#linear-assignment-of-gcd-then-match-the-cores).
...
["Linear ssignment of GCD, then match the cores" strategy](http://localhost:8000/LUMI-BE-training-materials/intro-evolving/07-Binding/#linear-assignment-of-gcd-then-match-the-cores).
